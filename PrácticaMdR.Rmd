...
plot(datos$temp, datos$area, main = "Regresión polinómica de segundo grado", pch = 19, col = "blue")
...
plot(df_filtered$temp, df_filtered$area, main = "Regresión Polinómica de Segundo Grado", pch = 19, col = "blue")
...
Como hemos podido ver, un modelo polinómico no es mejor que un modelo lineal para nuestros datos, es por ello que vamos a utilizar un modelo logarítmico, sumando 1 a nuestra variable objetivo ya que tenemos ceros en esta y si no, no se podrá hacer el logaritmo. Además, la transformación logarítmica nos ayuda a lidiar con los dos outliers que tenemos, haciendo que influyan menos en el modelo:
...
Vemos que los residuos son homocedásticos, vamos a ver si siguen una distribución normal:
...
Los residuos no son homocedásticos, comprobamos su normalidad:
...
Vemos que en este caso los residuos no son homocedásticos, por lo que el modelo no nos sirve.
...
Vemos que son my similares a una distriibución normal. A pesar de esto, como no son homocedásticos no es un modelo que nos sirva. Además, en el resumen del modelo vemos como tenemos un R² negativo, lo cual nos dice que el modelo no es bueno
...
Podemos ver cómo los residuos no son homocedásticos pero están cerca de serlo. Comprobamos ahora la normalidad de los mismos:
...
Podemos ver como el modelo con la transformación yeo-johnson eliminando los ceros y utilizando ISI tiene el R² y el R² ajustado más altos, por lo que el ajuste es mejor en este modelo. A pesar de esto, tiene también el valo más alto de RMSE, lo cual nos dice que tiene muchos residuos, en este caso el mejor modelo es el polinómico. En cuanto al AIC y BIC, el peor modelo es el Yeo-Johnson. Teniendo todo esto en cuenta, además del análisis anterior de los residuos de cada uno de los modelos, podemos decir que el modelo con la transformación yeo-johnson eliminando cerosy utilizando la variable ISI es el mejor, al menos en cuanto a la distribución y características de los resíduos y el R², tanto el ajustado como el normal.
