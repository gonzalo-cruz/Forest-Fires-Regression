...
```{r}
# Crear una columna month_num
datos$month_num <- as.numeric(factor(datos$month, levels = c("jan","feb","mar","apr","may","jun",
                                                           "jul","aug","sep","oct","nov","dec")))
```


```{r}
gam_time <- gam(area ~ s(temp) + te(X, Y, month_num, k = c(5, 5, 6)),
                data = datos,
                family = gaussian(link = "log"))
summary(gam_time)
```
...
```{r}
gam_time_better <- gam(area ~ s(temp) + 
                   ti(X, Y, k = 5) +          # Efecto espacial
                   ti(month_num, k = 6) +      # Efecto temporal
                   ti(X, Y, month_num, k = c(5, 5, 6)),  # Interacción
                 data = datos,
                 family = gaussian(link = "log"))
summary(gam_time_better)
```

```{r}
plot(gam_time_better, select = 3)  # Para ver el efecto ti(month_num)
```
