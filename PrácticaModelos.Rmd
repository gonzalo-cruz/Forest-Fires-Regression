...
A continuación vamos a hacer modelos GAM con nuestro dataset sin ceros, para ver cómo estos modelos son capaces de predecir el área quemada cuando sabemos que hay un incendio. 
...
leaflet(datos) %>% 
  addTiles() %>% 
  addHeatmap(
    lng = ~X,        # Usamos X como longitud (si son coordenadas reales)
    lat = ~Y,        # Usamos Y como latitud
    intensity = ~area,
    radius = 10,      # Controla el tamaño de los puntos de calor
    blur = 15,        # Suaviza la transición entre puntos
    minOpacity = 0.1, # Opacidad mínima (para áreas de baja intensidad)
    max = max(datos$area, na.rm = TRUE)  # Escala el valor máximo de intensidad
  )
```
...
Tras comparar los tres modelos (regresión logística, Poisson y binomial negativo), podemos decir lo siguiente:
...
Modelo Binomial Negativo: este modelo, parece ser el que mejor se ajusta a nuestros datos. La dispersión de 0.1654 indica que el modelo binomial negativo maneja mejor la sobredispersión. Además, los coeficientes del modelo son coherentes y el AIC de 2682.6 es relativamente bajo, lo que sugiere un buen ajuste.

En nuestro dataset los incendios solo vienen representados por las coordenadas X e Y y su área, además del resto de variables climáticas e índices FWI, pero si se incluyeran variables adicionales como un ID por incendio o si su origen es natural o provocado daría información adicional que podrían mejorar la predicción de estos.
