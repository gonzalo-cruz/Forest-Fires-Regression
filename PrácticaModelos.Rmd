...
Para celdas con suficientes incendios, se ajustaron modelos logísticos que predicen la probabilidad de incendio (área > 0) en función de condiciones meteorológicas. La calidad de cada modelo se evaluó usando la matriz de confusión, precisión, sensibilidad (capacidad para detectar incendios reales), especificidad (capacidad para evitar falsas alarmas) y el AUC (capacidad discriminativa). Esta información permite identificar qué zonas del parque son más predecibles y bajo qué condiciones.
...
# Filtrar celdas con al menos 25 observaciones
conteos <- datos %>%
  group_by(X, Y) %>%
...
# Loop por cada celda
for (i in 1:nrow(conteos)) {
  x_val <- conteos$X[i]
  y_val <- conteos$Y[i]
  
  df1 <- datos %>% filter(X == x_val, Y == y_val)
  y <- df1$area > 0
  
  # Verificamos que haya clases positivas y negativas
  if (length(unique(y)) == 2) {
    modelo <- glm(y ~ (temp) + (RH ), data = df1, family = binomial)
    probas <- predict(modelo, type = "response")
    pred_clase <- ifelse(probas > 0.5, 1, 0)
    
    # Matriz de confusión
    conf_matrix <- table(Predicted = pred_clase, Actual = y)
    
    # Extraer VP, VN, FP, FN (con protección por si faltan)
    VP <- ifelse("1" %in% rownames(conf_matrix) & "TRUE" %in% colnames(conf_matrix), conf_matrix["1", "TRUE"], 0)
    VN <- ifelse("0" %in% rownames(conf_matrix) & "FALSE" %in% colnames(conf_matrix), conf_matrix["0", "FALSE"], 0)
    FP <- ifelse("1" %in% rownames(conf_matrix) & "FALSE" %in% colnames(conf_matrix), conf_matrix["1", "FALSE"], 0)
    FN <- ifelse("0" %in% rownames(conf_matrix) & "TRUE" %in% colnames(conf_matrix), conf_matrix["0", "TRUE"], 0)
    
    accuracy <- (VP + VN) / sum(conf_matrix)
    sensibilidad <- ifelse((VP + FN) > 0, VP / (VP + FN), NA)
    especificidad <- ifelse((VN + FP) > 0, VN / (VN + FP), NA)
    
    # AUC (puede fallar si no hay clases distintas)
    auc_val <- tryCatch({
      roc_obj <- roc(y, probas)
...
# Mostrar resultados ordenados por AUC
